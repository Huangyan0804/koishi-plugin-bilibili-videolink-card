...
class BilibiliParser {
    ...
    // 添加任务到缓冲区（已废弃，保留兼容性）
    async processVideoFromLink(session, ret, options = {
        video: true,
    }) {
        // 将任务加入缓冲队列
    ...
    // 实际处理单个视频任务
    async processVideoTask(session, ret, options = {
        video: true,
    }) {
            ...
            try {
                ...
                if (responseData?.code === 200 &&
                    ...
                    if (videoDurationMinutes < this.config.Minimumduration) {
                        ...
                        else if (typeof this.config.Minimumduration_tip === "object" &&
                    ...
                    // 检查视频是否太长
                        ...
                        else if (typeof this.config.Maximumduration_tip === "object" &&
                        ...
                        // 视频时长在允许范围内，处理视频
                        let videoData = videoUrl; // 初始为原始 URL
                        ...
                        if (fileTooLarge) {
                            ...
                            if (typeof this.config.Maximumduration_tip === "object" &&
                                this.config.Maximumduration_tip !== null) {
                                if (this.config.Maximumduration_tip.tipcontent) {
                                    if (this.config.Maximumduration_tip.tipanalysis) {
                                        // 提示语合并到消息中
                            ...
                            if (options.link) {
                                // 如果是链接选项，仍然使用原始URL
                                videoElements.push(koishi_1.h.text(videoUrl));
                            }
    ...
    generateCardHtml(info, danmaku, comments, tags, online, fontFamily) {
        ...
        const stats = statItems
            .map(([name, value]) => `<span class="stat">${this.icon(name)} ${value}</span>`)
            .join("");
        ...
        const commentsHtml = `<section><div class="section-title"><h2>热门评论</h2><span>${this.numeral(stat.reply || 0)} 条评论</span></div>${comments.length
            ? comments
                ...
                const levelBadge = level !== undefined && level !== null
