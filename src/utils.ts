...
const htmlHelpers: {
  fromHtml(html: string): { node: Node; stylesheets: string[] };
} = require("@takumi-rs/helpers/html");
const emojiHelpers: {
  extractEmojis(node: Node, type: "twemoji"): Node;
} = require("@takumi-rs/helpers/emoji");
...
export class BilibiliParser {
  private cardRenderer?: { renderer: Renderer; fontFamily: string };
  ...
  public isLinkProcessedRecently(ret: string, channelId: string) {
    ...
    if (
      lastretUrl &&
      this.lastProcessedUrls[channelKey] &&
      currentTime - this.lastProcessedUrls[channelKey] <
        this.config.MinimumTimeInterval * 1000
    ) {
  ...
  private flushSessionBuffer() {
    ...
    this.logInfo(
      `Session缓冲时间结束，开始处理 ${this.sessionQueue.length} 个消息`,
    );
  ...
  // 添加任务到缓冲区（已废弃，保留兼容性）
  public async processVideoFromLink(
    session: Session,
    ret: string,
    options: { video?: boolean; audio?: boolean; link?: boolean } = {
      video: true,
  ...
  private flushBuffer() {
    ...
    this.logInfo(
      `缓冲时间结束，将 ${this.bufferQueue.length} 个任务加入处理队列`,
    );
  ...
  private async processVideoTask(
    ...
    if (this.config.waitTip_Switch) {
      const result = await session.send(
        `${h.quote(session.messageId)}${this.config.waitTip_Switch}`,
      );
    ...
    let shouldPerformTextParsing =
      this.config.videoParseComponents.includes("text");

    // 先进行图文解析
    if (shouldPerformTextParsing && lastretUrl) {
      const videoLink = this.link_type_parser(lastretUrl).find(
    ...
    // 合并转发处理
    if (
      this.config.isfigure &&
      (session.platform === "onebot" || session.platform === "red")
    ) {
  ...
  private async fetchDanmaku(cid: number): Promise<string[]> {
    try {
      ...
      return Array.from(
        String(response).matchAll(/<d [^>]*>(.*?)<\/d>/g),
        (match) =>
          match[1]
            .replace(/&amp;/g, "&")
            .replace(/&lt;/g, "<")
  ...
  private async fetchComments(aid: number): Promise<any[]> {
    try {
      const response: any = await this.ctx.http.get(
  ...
  private async fetchOnline(aid: number, cid: number): Promise<string | null> {
    try {
      const response: any = await this.ctx.http.get(
  ...
  private async renderVideoCard(info: any): Promise<Buffer> {
    ...
    const parsed = htmlHelpers.fromHtml(
      this.generateCardHtml(info, danmaku, comments, tags, online, fontFamily),
    );
    const node = emojiHelpers.extractEmojis(parsed.node, "twemoji");
    const fetchedResources = await this.fetchCardResources(
      extractResourceUrls(node),
    );
  ...
  private getCardRenderer(): { renderer: Renderer; fontFamily: string } {
    if (this.cardRenderer) return this.cardRenderer;
  ...
  private async fetchCardResources(urls: string[]): Promise<ImageSource[]> {
    const resources: ImageSource[] = [];
    await Promise.all(
      urls.map(async (src) => {
        try {
  ...
  private renderCommentMessage(
    message: string,
    emotes: Record<string, { url?: string }> = {},
  ): string {
  ...
  private generateCardHtml(
    ...
    const stats = statItems
      .map(
        ([name, value]) =>
          `<span class="stat">${this.icon(name)} ${value}</span>`,
      )
      .join("");
    ...
    const danmakuHtml = danmaku.length
      ...
      : "";
    const commentsHtml = `<section><div class="section-title"><h2>热门评论</h2><span>${this.numeral(stat.reply || 0)} 条评论</span></div>${
      comments.length
        ? comments
            .map((comment) => {
...
`;
  ...
  private icon(name: string): string {
    const icons: Record<string, string> = {
      ...
      online:
        '<svg viewBox="0 0 24 24"><path d="M12 5c5.5 0 9.3 5.1 9.5 5.3.4.4.4 1 0 1.4C21.3 11.9 17.5 17 12 17S2.7 11.9 2.5 11.7a1 1 0 0 1 0-1.4C2.7 10.1 6.5 5 12 5Zm0 2c-3.6 0-6.5 2.9-7.4 4 .9 1.1 3.8 4 7.4 4s6.5-2.9 7.4-4c-.9-1.1-3.8-4-7.4-4Zm0 1.5a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Z"/></svg>',
      ...
      share:
        '<svg viewBox="0 0 24 24"><path d="M12.5 4.2c-.7-.5-1.7 0-1.7.9v3.4C6.2 9 3.1 12.2 2.1 17.2c-.2 1 .9 1.6 1.6.8 1.8-2.2 4.2-3.4 7.1-3.7v3.4c0 .9 1 1.4 1.7.8l8.4-6.4c.6-.5.6-1.4 0-1.9z"/></svg>',
    };
    const icon = icons[name];
    if (!icon) return "";
    const svg = icon.replace(
      "<svg ",
      '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#61666d" ',
    );
   ...
   * @returns 解析来的文本
   */
  private async type_processer(element: {
    type: string;
    id: string;
  }): Promise<string> {
    var ret = "";
    switch (element["type"]) {
      ...
      case "Short":
        const typed_link = this.link_type_parser(
          await this.get_redir_url(element["id"]),
        );
