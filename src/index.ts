...
export interface Config {
  ...
  Minimumduration_tip:
    | "return"
    | { tipcontent: string; tipanalysis: boolean }
    | null;
...
export const Config = Schema.intersect([
  // 解析功能总开关
  Schema.object({
    enablebilianalysis: Schema.boolean()
      .default(true)
    ...
    Schema.intersect([
      ...
      Schema.object({
        ...
        videoParseMode: Schema.array(
          ...
          .role("checkbox")
          .description("选择解析来源"),
        videoParseComponents: Schema.array(
          Schema.union([
      ...
      // 视频过滤规则
      Schema.object({
        MinimumTimeInterval: Schema.number()
          .default(180)
          .description(
            "若干`秒`内 不再处理相同链接 `防止多bot互相触发 导致的刷屏/性能浪费`",
          )
          .min(1),
        Minimumduration: Schema.number()
          .default(0)
          .description(
            "允许解析的视频最小时长（分钟）`低于这个时长 就不会发视频内容`",
          )
          .min(0),
        Minimumduration_tip: Schema.union([
          Schema.const("return").description("不返回文字提示"),
          Schema.object({
            tipcontent: Schema.string()
              .default("视频太短啦！不看不看~")
              .description("文字提示内容"),
            tipanalysis: Schema.boolean()
              .default(true)
          ...
          .default(null),
        Maximumduration: Schema.number()
          .default(25)
          .description(
            "允许解析的视频最大时长（分钟）`超过这个时长 就不会发视频内容`",
          )
          .min(1),
        Maximumduration_tip: Schema.union([
          ...
          .default(null),
        MaximumFileSizeMB: Schema.number()
          .default(50)
          .description(
            "文件缓冲最大大小（MB）`超过这个大小 就不会发视频内容`<br>设置为0 表示不限制",
          )
          .min(0)
          .max(200),
      }).description("视频过滤规则"),
      // 图文解析设置
      Schema.object({
        bVideoShowLink: Schema.boolean()
          .default(false)
          .description(
            "在末尾显示视频的链接地址 `开启可能会导致其他bot循环解析`",
          ),
      }).description("图文解析设置"),
      // 高级功能设置
      Schema.object({
        isfigure: Schema.boolean()
          .default(false)
          .description(
            "是否开启合并转发 `仅支持 onebot 适配器` 其他平台开启 无效",
          )
          .experimental(),
        filebuffer: Schema.boolean()
          .default(true)
          .description(
            "是否将视频链接下载后再发送 （以解决部分onebot协议端的问题）<br>否则使用视频直链发送",
          )
          .experimental(),
        bufferDelay: Schema.number()
          .default(5)
          .description(
      ...
      // 隐藏配置项
      Schema.object({
        parseLimit: Schema.number()
          .default(3)
          .description("单对话多链接解析上限")
          .hidden(),
        useNumeral: Schema.boolean()
          .default(true)
          .description("使用格式化数字")
          .hidden(),
  ...
  Schema.object({
    loggerinfo: Schema.boolean()
      ...
      .experimental(),
    loggerinfofulljson: Schema.boolean()
      .default(false)
